...
雨舒 Core 是一个面向 AstrBot 的 owner 私聊陪练插件。v0.1.x 聚焦“可控、可回滚、可管理”的最小可用能力：owner 私聊提示词注入、短记忆摘要、复盘模式、手动记忆管理，以及 AstrBot WebUI 管理页面。

> 设计边界：雨舒只在配置的 owner 私聊中启用陪练能力；群聊保持普通 bot/群友型行为，不注入 owner 私聊记忆。

## 快速开始

1. 下载 `astrbot_plugin_yushu_core-v0.1.1.tar.gz`，解压到 `AstrBot/data/plugins/astrbot_plugin_yushu_core`。
...
- 需要 AstrBot 支持 `on_agent_done`，用于将复盘回复标记为临时消息，避免进入后续角色内上下文。
...
- 复盘不进历史：复盘模式的 assistant 回复默认标记为临时消息，避免污染后续角色内聊天上下文。
